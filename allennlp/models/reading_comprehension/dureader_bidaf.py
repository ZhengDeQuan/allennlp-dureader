...
class BidirectionalAttentionFlow(Model):
    ...
    def apply_concat_mask_fn(self, vector, mask, batch_size, fn):
        """
        ...
        """
        fake_batch_size, seq_len = vector.shape
        assert fake_batch_size % batch_size == 0
        p_num = fake_batch_size // batch_size
        vector = vector.view(batch_size, -1)
        if mask is None:
            result = torch.nn.functional.softmax(vector, dim=-1)
        else:
            mask = mask.view(batch_size, -1)
            assert vector.shape == mask.shape, '{} vs {}'.format(vector.shape, mask.shape)
            # To limit numerical errors from large vector elements outside the mask, we zero these out.
            vector = vector + (1-mask)*(-1e30)
            #result = torch.nn.functional.softmax(vector, dim=-1)
            result = fn(vector, dim=-1)
            result = result * mask
        result = result.view(fake_batch_size, -1)
        return result
    ...
    @staticmethod
    def get_best_span(span_start_logits: Variable, span_end_logits: Variable):
        if span_start_logits.dim() != 2 or span_end_logits.dim() != 2:
            raise ValueError("Input shapes must be (batch_size, passage_length)")
        batch_size, passage_length = span_start_logits.size()
        max_span_log_prob = np.array([-1e20] * batch_size)
        span_start_argmax = [0] * batch_size
        # best_word_span = Variable(span_start_logits.data.new()
        #                           .resize_(batch_size, 2).fill_(0)).long()
        best_word_span = np.zeros((batch_size, 2), dtype = np.int64)
        ...
        return best_word_span, max_span_log_prob
